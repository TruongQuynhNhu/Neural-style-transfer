...
- inside content loss and style loss, we have the normalized number of each loss layer. Which is: $\frac{1}{4 \times n_H \times n_W \times n_C}$. They are not really important because after that, we also have weight of each layer (layer_weights) and the weight of content loss and style loss in the total loss (alpha and beta). However, I recognize that, using it would make the loss being normalized to proper scale, make it easy for us to track and fine-tune the other hyper parameter to get our desired result.
